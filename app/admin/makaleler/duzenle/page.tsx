...
import { getArticleFile, createOrUpdateFile, buildMdxContent } from '@/lib/github-api'
...
function EditArticleInner() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const newPath = `content/articles/${updatedFormData.category}/${slug}.mdx`
      const oldPath = `content/articles/${originalCategory}/${slug}.mdx`

      let newSha = sha
      if (updatedFormData.category !== originalCategory) {
        const { deleteFile } = await import('@/lib/github-api')
        newSha = await createOrUpdateFile(newPath, mdx, undefined, `chore: move ${slug} to ${updatedFormData.category}`)
        await deleteFile(oldPath, sha, `chore: remove ${slug} from ${originalCategory}`)
        setOriginalCategory(updatedFormData.category)
      } else {
        newSha = await createOrUpdateFile(oldPath, mdx, sha, `chore: update ${slug}`)
      }
      setSha(newSha)
      setFormData(updatedFormData)
      setVisualContent(markdownToHtml(currentContent))
      ...
      <div className="flex items-center justify-between">
        <button
          onClick={() => {
            ...
            router.push(`/admin/makaleler?category=${formData.category}&highlight=${slug}`)
          }}
