  ...
  return (
    <>
      ...
      >
        ...
        {/* Nav links — shifted left, between logo and right edge */}
        <div className="hidden lg:flex items-center gap-6 flex-1 justify-center">
          {navLinks.map(({ href, en, tr }) => (
            <Link key={href} href={href} className="hover:text-gold transition-colors text-center group">
              <span className="block text-white text-[11px] font-sans tracking-[1px] group-hover:text-gold transition-colors">{en}</span>
              {tr && <span className="block text-white/35 text-[8px] font-sans tracking-[0.5px] mt-0.5 group-hover:text-gold/60 transition-colors">{tr}</span>}
            </Link>
          ))}
        </div>

        {/* Center logo */}
        <div className="text-center flex-shrink-0 ml-auto hidden lg:block">
          <Link href="/">
